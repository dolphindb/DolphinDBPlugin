...
Signal 插件目前支持版本：[release200](https://github.com/dolphindb/DolphinDBPlugin/blob/release200/signal/README_CN.md) 和 [release130](https://github.com/dolphindb/DolphinDBPlugin/blob/release130/signal/README_CN.md)。您当前查看的插件版本为 release130，请使用 DolphinDB 1.30.X 版本 server。若使用其它版本 server，请切换至相应插件分支。
...
* 修复了频繁调用 signal::secc 等接口时内存泄漏的问题。（**1.30.22**）
* 修复多线程并发访问 signal::fft, signal::ifft, signal::secc 等接口时 Signal 插件宕机的问题。（**1.30.22**）
* 修复当 signal::fft 函数的输入数据为偶数个时，输出值的正负符号错误的问题。（**1.30.22**）
