                        ...
                        switch(btype){
                            case BSON_TYPE_DOUBLE:{
                                ...
                                if(rNum<mIndex){
                                    ...
                                    for(long long j=0;j<rwNum;++j){
                                        reinterpret_cast<VectorSP&>(cols[i])->appendDouble((double*)buffer[i],mIndex);
                                    }
                                }
                                *((double*)buffer[i]+rNum%mIndex)=bson_iter_double(&biter);
                                nullMap.erase(i);
                                break;
                            }
                            case BSON_TYPE_UTF8:{
                                ...
                                if(rNum>=mIndex){
                                    ...
                                    long long rwNum=rNum/mIndex;
                                    for(long long j=0;j<rwNum;++j){
                                        reinterpret_cast<VectorSP&>(cols[i])->appendString(tmp.data(),mIndex);
                                    }
                                }
                            ...
                            case BSON_TYPE_INT32:{
                                ...
                                if(rNum<mIndex){
                                    ...
                                    long long rwNum=rNum/mIndex;
                                    for(long long j=0;j<rwNum;++j){
                                        reinterpret_cast<VectorSP&>(cols[i])->appendInt((int*)buffer[i],mIndex);
                                    }
                                }
                                *((int*)buffer[i]+rNum%mIndex)=bson_iter_int32(&biter);
                                nullMap.erase(i);
                                break;
                            }
                            case BSON_TYPE_INT64:{
                                ...
                                if(rNum<mIndex){
                                    for(long long j=0;j<rNum;++j){
                                        *((long long* )buffer[i]+j)=LONG_LONG_MIN;
                                    }
                                }
                                else{
                                    for(long long  j=0;j<mIndex;++j){
                                    ...
                                    long long rwNum=rNum/mIndex;
                                    for(long long j=0;j<rwNum;++j){
                                        reinterpret_cast<VectorSP&>(cols[i])->appendLong((long long*)buffer[i],mIndex);
                                    }
                                }
                                *((long long*)buffer[i]+rNum%mIndex)=bson_iter_int64(&biter);
                                nullMap.erase(i);
                                break;
                            }
                            case BSON_TYPE_OID:{
                            ...
                            case BSON_TYPE_BOOL:{
                                ...
                                if(rNum<mIndex){
                                    ...
                                    long long rwNum=rNum/mIndex;
                                    for(long long j=0;j<rwNum;++j){
                                        reinterpret_cast<VectorSP&>(cols[i])->appendBool((char*)buffer[i],mIndex);
                                    }
                                }
                                *((bool*)buffer[i]+rNum%mIndex)=bson_iter_bool(&biter);
                                nullMap.erase(i);
                                break;
                            }
                            case BSON_TYPE_DATE_TIME:{
                                ...
                                *((long long*)buffer[i]+rNum%mIndex)=bson_iter_date_time(&biter);
                                nullMap.erase(i);
                                break;
                            }
                            case BSON_TYPE_DECIMAL128:{
                                ...
                                try{
                                    double t=std::stod(ttmp);
                                    *((double*)buffer[i]+rNum%mIndex)=t;
                                }
                                catch(std::exception& e){
                                    *((double*)buffer[i]+rNum%mIndex)=DBL_NMIN;
                                }
                                nullMap.erase(i);
                                break;
                            }
                        ...
                        switch (dtype) {
                            case DT_BOOL:
                                switch (btype) {
                                    ...
                                    default:
                                        throw RuntimeException("Cannot convert from Mongodb " + getBsonString(btype) +
                                                               " to DolphinDB " + Util::getDataTypeString(dtype));
                                }
                                break;
                            case DT_CHAR:
                                switch (btype) {
                                    ...
                                    case BSON_TYPE_DOUBLE: {
                                        double t = bson_iter_double(&biter);
                                        *((char *)(buffer[i]) + index) = std::llround(t);
                                        break;
                                    }
                                    case BSON_TYPE_DECIMAL128: {
                                        bson_decimal128_t dec;
                            ...
                            case DT_SHORT:
                                switch (btype) {
                                    ...
                                    case BSON_TYPE_DOUBLE: {
                                        double t = bson_iter_double(&biter);
                                        *((short *)(buffer[i]) + index) = std::llround(t);
                                        break;
                                    }
                                    case BSON_TYPE_DECIMAL128: {
                                        bson_decimal128_t dec;
                            ...
                            case DT_INT: {
                                switch (btype) {
                                    ...
                                    case BSON_TYPE_DOUBLE: {
                                        double t = bson_iter_double(&biter);
                                        *((int *)(buffer[i]) + index) = std::llround(t);
                                        break;
                                    }
                                    case BSON_TYPE_DECIMAL128: {
                                        bson_decimal128_t dec;
                            ...
                            case DT_LONG: {
                                switch (btype) {
                                    ...
                                    case BSON_TYPE_DOUBLE: {
                                        double t = bson_iter_double(&biter);
                                        *((long long *)(buffer[i]) + index) = std::llround(t);
                                        break;
                                    }
                                    case BSON_TYPE_DECIMAL128: {
                                        bson_decimal128_t dec;
                            ...
                            default: {
                                throw RuntimeException("The dolphindb type " + Util::getDataTypeString(dtype) +
                                                       " is not supported");
                            }
                        }
                    }
