        ...
        try {
            ...
            if (LIKELY(parser_->getType() == DT_FUNCTIONDEF)) {
                ...
                if (parser_->hasMethod(name)) {
                    FunctionDefSP parseAndHandle = parser_->getMethod(name);
                    vector<ConstantSP> args = {parser_, content};
                    parseAndHandle->call(session_->getHeap().get(), args);
                } else {
                    throw RuntimeException("cannot find parseAndHandle in coder instance.");
                }
            }
