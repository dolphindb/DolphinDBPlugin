    ...
    while(LIKELY(flag_&& consumer)){
        ...
        if(msg){
            try {
                ...
                } else if(parser_->getForm() == DF_SYSOBJ) {
                    vector<ConstantSP> tableArgs = {parserString[0]};
                    ConstantSP content = Util::createTable({"string"}, tableArgs);
                    string name = "parseAndHandle";
                    if (parser_->hasMethod(name)) {
                        FunctionDefSP func = parser_->getMethod(name);
                        vector<ConstantSP> parseArgs{parser_, content};
                        func->call(session_->getHeap().get(), parseArgs);
                    } else {
                        LOG_ERR("[PLUGIN::KAFKA] handle message failed, cannot find 'parseAndHandle' method.");
                    }
                } else {
                    std::cout << parser_->getForm() <<"\n";
