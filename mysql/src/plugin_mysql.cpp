...
ConstantSP mysqlConnect(Heap *heap, vector<ConstantSP> &args) {
    std::string usage = "Usage: connect(host, user, password, port, db). ";
    // parse args first
    ...
    std::unique_ptr<Connection> cup(new Connection(args[0]->getString(), args[1]->getString(), args[2]->getString(),
                                                   args[3]->getInt(), args[4]->getString()));
...
ConstantSP mysqlLoad(Heap *heap, vector<ConstantSP> &arguments) {
    ...
    std::string usage = "Usage: load(connection, table_or_query, [schema], [startRow=0], [rowNum=INT_MAX]). ";

    std::string table;
    TableSP schema = nullptr;
    int startRow = 0, rowNum = INT_MAX;
    ...
    if (!args[3]->isNull()) {
        if (args[3]->getType() != DT_INT || args[3]->getForm() != DF_SCALAR)
            throw IllegalArgumentException(__FUNCTION__, usage + "startRow must be a integer");
        if (args[3]->getInt() < 0)
            throw IllegalArgumentException(__FUNCTION__, usage + "startRow must be a non-negative integer");
        startRow = args[3]->getInt();
    }
...
ConstantSP mysqlLoadEx(Heap *heap, vector<ConstantSP> &arguments) {
    ...
    std::string usage = "Usage: loadEx(connection, "
                        "dbHandle,tableName,[partitionColumns],tableInMySQL_or_query,[schema],[startRow=0],[rowNum=INT_MAX]) ";
    ...
    if (!args[3]->isNull()) {
        if (args[3]->getType() != DT_STRING && args[3]->getRawType() != DT_STRING)
...
Connection::Connection(std::string hostname, std::string username, std::string password, int port, std::string database)
    : host_(hostname), user_(username), password_(password), db_(database) {
    try {
        connect(db_.c_str(), host_.c_str(), user_.c_str(), password_.c_str(), port);
...
ConstantSP Connection::load(const std::string &table_or_query, const TableSP &schema, const int &startRow,
                            const int &rowNum) {
    if (isQuery(table_or_query)) {
        return MySQLExtractor(query(table_or_query)).extract(schema, (size_t) startRow);
    } else {
        if (rowNum == INT_MAX)
            return MySQLExtractor(query("SELECT * FROM " + table_or_query)).extract(schema, (size_t) startRow);
        else
            return MySQLExtractor(
                query("SELECT * FROM " + table_or_query + " LIMIT "
                          + std::to_string((size_t) startRow + (size_t) rowNum)))
                .extract(schema, (size_t) startRow);
    }
}
...
ConstantSP Connection::loadEx(Heap *heap, const ConstantSP &dbHandle, const std::string &tableName,
                              ...
                              const TableSP &schema, const int &startRow, const int &rowNum) {
    ...
    if (tableExists) {
        // must use new String(addr) here, otherwise pointer in ConstantSP is freed by someone for whatever reason.
        vector<ConstantSP> args{new String(addr), new String(tableName)};
    ...
    if (isQuery(MySQLTableName_or_query)) {
        MySQLExtractor(query(MySQLTableName_or_query)).extractEx(heap, ret, schema, (size_t) startRow);
    } else {
        if (rowNum == INT_MAX) {
            MySQLExtractor(query("SELECT * FROM " + MySQLTableName_or_query)).extractEx(heap,
                                                                                        ret,
                                                                                        schema,
                                                                                        (size_t) startRow);
        } else {
            MySQLExtractor(
                query("SELECT * FROM " + MySQLTableName_or_query + " LIMIT "
                          + std::to_string((size_t) startRow + (size_t) rowNum)))
                .extractEx(heap, ret, schema, (size_t) startRow);
        }
    }
...
MySQLExtractor::MySQLExtractor(const mysqlxx::Query &q)
    : query_(q), emptyPackIdx_(DEFAULT_WORKSPACE_SIZE), fullPackIdx_(DEFAULT_WORKSPACE_SIZE),
...
TableSP MySQLExtractor::extractSchema(const std::string &table) {
    try {
        ...
        for (int i = 0; i < numFields; ++i) {
            ...
            colTypes->set(i, new String(getDolphinDBTypeStr(getDolphinDBType(res.typeAt(i)))));
...
#else
        return Util::createTable({"name", "type", "MySQL describe type"},
                                 {colNames, colTypes, colTypesMySQL});
#endif
    } catch (mysqlxx::Exception &e) {
...
TableSP MySQLExtractor::extract(const ConstantSP &schema, const size_t startRow) {
    try {
        auto res = query_.use();
        prepareForExtract(schema, res);
        TableSP t = Util::createTable(colNames_, dstColTypes_, 0, 0);

        realExtract(res, schema, startRow, t, [&](Pack &p) { growTable(t, p); });
        if (t->size() == 0) {
...
void MySQLExtractor::extractEx(Heap *heap, TableSP &t, const ConstantSP &schema, const size_t startRow) {
    auto res = query_.use();
    prepareForExtract(schema, res);
    realExtract(res, schema, startRow, t, [&](Pack &p) { growTableEx(t, p, heap); });
}

void MySQLExtractor::realExtract(mysqlxx::UseQueryResult &res, const ConstantSP &schema, const size_t startRow,
                                 TableSP &resultTable, std::function<void(Pack &)> callback) {
    try {
...
vector<DATA_TYPE> MySQLExtractor::getColTypes(mysqlxx::ResultBase &res) {
    ...
    for (size_t i = 0; i < nCols; ++i) {
        ret[i] = getDolphinDBType(res.typeAt(i));
    }
    return ret;
}
...
vector<size_t> MySQLExtractor::getMaxStrlen(mysqlxx::ResultBase &res) {
    ...
    for (int i = 0; i < numFields; ++i) {
        ...
        if (t == mysqlxx::MYSQL_TYPE_STRING || t == mysqlxx::MYSQL_TYPE_VAR_STRING ||
            t == mysqlxx::MYSQL_TYPE_VARCHAR || t == mysqlxx::MYSQL_TYPE_ENUM || t == mysqlxx::MYSQL_TYPE_BLOB ||
            t == mysqlxx::MYSQL_TYPE_TINY_BLOB || t == mysqlxx::MYSQL_TYPE_MEDIUM_BLOB ||
            t == mysqlxx::MYSQL_TYPE_LONG_BLOB) {
            if (ret[i] % 3 != 0)
                throw RuntimeException("must be multiple of 3");
            ret[i] /= 3;
            ret[i] += 1;
        }
    }
...
/// Pack members
Pack::Pack(vector<DATA_TYPE> srcDt, vector<DATA_TYPE> dstDt, vector<size_t> maxStrLen, TableSP &resultTable,
           size_t cap) {
...
void Pack::init(vector<DATA_TYPE> srcDt, vector<DATA_TYPE> dstDt, TableSP &resultTable, vector<size_t> maxStrLen,
                size_t cap) {
    ...
    if (capacity_ == 0) {
        throw RuntimeException("Insufficient memory to load even one row, please check your MySQL table.");
    }
...
DATA_TYPE getDolphinDBType(mysqlxx::enum_field_types type) {
    ...
    switch (type) {
        ...
        default:
            throw IllegalArgumentException(__FUNCTION__,
                                           "MySQL type " + std::string(getMySQLTypeStr(type))
                                               + " not supported yet.");
    }
}
...
bool compatible(DATA_TYPE dst, DATA_TYPE src) {
    ...
    // set<DATA_TYPE > time{DT_TIME, DT_TIMESTAMP, DT_NANOTIME, DT_NANOTIMESTAMP};
    // from TIMESTAMP to DATETIME, DATE, TIME, NANODATETIME, NANOTIMESTAMP, NANOTIME
    // from DATETIME to TIMESTAMP, DATE, TIME, NANODATETIME, NANOTIMESTAMP, NANOTIME
