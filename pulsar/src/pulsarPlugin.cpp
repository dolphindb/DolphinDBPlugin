...
ConstantSP pulsarCreateSubJob(Heap *heap, argsT &args) {
    ...
    if (args[4]->getForm() == DF_TABLE) {
        ...
        if (table->getTableType() != REALTIMETBL or !table->isSharedTable()) {
            throw IllegalArgumentException(__FUNCTION__, usage + "the 5th parameter should be a shared stream table or a string scalar.");
        }
    } else if(args[4]->getForm() == DF_SCALAR && args[4]->getType() == DT_STRING) {
        std::string orca = args[4]->getString();
        if(orca.empty()) {
            throw IllegalArgumentException(__FUNCTION__, usage + "orca cannot be empty");
        }
        FunctionDefSP loadOrcaFunc = heap->currentSession()->getFunctionDef("loadOrcaStreamTable");//->call(session->getHeap().get(), args);
        if(loadOrcaFunc.isNull()) {
            throw IllegalArgumentException(__FUNCTION__, usage + "not support loadOrcaStreamTable");
        }
        ConstantSP name = new String(orca);
        vector<ConstantSP> loadArgs = {name};
        table = loadOrcaFunc->call(heap, loadArgs);
    } else {
        throw IllegalArgumentException(__FUNCTION__, usage + "the 5th parameter should be a shared stream table or a string scalar.");
    }
    // parser
...
ConstantSP pulsarGetJobStat(Heap *heap, argsT &args) {
    ...
    argsT cols = {nameVec, userVec, createTimeVec, endTimeVec};
