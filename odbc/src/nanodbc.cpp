...
inline void result::result_impl::get_ref_impl<string>(short column, string& result) const
{
    ...
    {
        std::string buffer;
        buffer.reserve(column_size + 2); // ensure terminating null & negative sign
        buffer.resize(buffer.capacity());
        using std::fill;
        fill(buffer.begin(), buffer.end(), '\0');
        const int32_t data = *reinterpret_cast<int32_t*>(col.pdata_ + rowset_position_ * col.clen_);
        const int bytes =
            std::snprintf(const_cast<char*>(buffer.data()), column_size + 2, "%d", data);
        if (bytes == -1)
            throw type_incompatible_error();
        else if ((SQLULEN)bytes < column_size + 1)
            buffer.resize(bytes);
    ...
    case SQL_C_SBIGINT:
    {
        ...
        const int bytes =
            std::snprintf(const_cast<char*>(buffer.data()), column_size + 2, "%jd", data);
        if (bytes == -1)
            throw type_incompatible_error();
        else if ((SQLULEN)bytes < column_size + 1)
            buffer.resize(bytes);
