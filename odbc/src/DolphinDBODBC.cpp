...
static void odbcConnectionOnClose(Heap* heap, vector<ConstantSP>& args) {
    ...
    if (cp != nullptr) {
        ...
        // printf("connection %p already deleted", cp);
    }
}
...
static TableSP odbcGetOrCreateTable(const Heap* heap,
    ...
    if (args.size() >= 3) {
        ...
        // printf("return table, %d symbol vector(s)\n", symbolCount);
        // printf("return args[2]\n");
        return tblSP;
    } else {
        ...
        for (short i = 0; i < columns; ++i) {
            ...
            if (columnTypes[i] == DT_SYMBOL) {
                symbolCount++;
                // printf("column %s has size %ld, use symbol\n",
                // columnNames[i].c_str(), results.column_size(i));
            }
        }
        ...
        // printf("Util::createTable, %d symbol vector(s)\n", symbolCount);
...
ConstantSP odbcQuery(Heap* heap, vector<ConstantSP>& args) {
    ...
    while (hasNext) {
        ...
        if (hasNext == false || curLine == Util::BUF_SIZE) {
            ...
            // printf("appended %d rows\n", curLine);
            curLine = 0;
        }
    }
    for (short i = 0; i < columns; ++i) {
        arrCol[i]->setNullFlag(arrCol[i]->hasNull());
        // printf("total %d rows, %d rows appended at column %s\n", rowIdx,
        // arrCol[i]->size(), table->getColumnName(i).c_str());
    }
