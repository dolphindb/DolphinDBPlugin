...
static ConstantSP odbcGetConnection(Heap* heap, vector<ConstantSP>& args, const string& funcName) {
    if (args[0]->getType() == DT_STRING) {
        ...
        try {
            ...
            FunctionDefSP onClose(Util::createSystemProcedure("odbc connection onClose()", odbcConnectionOnClose, 1, 1));
            Constant* res = Util::createResource((long long)cup.release(), descBuf.data(), onClose, heap->currentSession());
            return res;
        ...
        throw IllegalArgumentException(funcName,"Unknown argument type, must be a connection string or odbc connection handle");
    }
}

static TableSP odbcGetOrCreateTable(const Heap* heap, const vector<ConstantSP>& args, const nanodbc::result& results, vector<ConstantSP>& columnVecs) {
    int symbolCount = 0;
    if (args.size() >= 3 && args[2]->getType() != DT_VOID) {
        ...
        if (columns != results.columns()) {
            throw TableRuntimeException("The given table schema is incompatible with the returned table from ODBC (different column count)");
        }
        for (INDEX i = 0; i < columns; ++i) {
            ...
            if (!compatible(dolphinType, results.column_datatype(i), results.column_size(i))) {
                const char* fmt = "The given table schema is incompatible with the returned table from ODBC at column %d[%s]";
            ...
            if (dolphinType == DT_SYMBOL) {
                columnVecs[i] = Util::createSymbolVector(col->getSymbolBase(), 0);
                symbolCount++;
            } else {
...
ConstantSP odbcClose(Heap* heap, vector<ConstantSP>& args) {
    ...
    if (args[0]->getType() != DT_RESOURCE) {
        throw IllegalArgumentException("odbc::close", "Unknown argument type, must be an odbc connection handle");
    }
...
ConstantSP odbcConnect(Heap* heap, vector<ConstantSP>& args) {
    ...
    assert(args.size() == 1);
    if (args[0]->getType() != DT_STRING) {
        throw IllegalArgumentException("odbc::connect", "Unknown argument type, must be an odbc connection string");
    }
...
ConstantSP odbcQuery(Heap* heap, vector<ConstantSP>& args) {
    ...
    if(args.size() == 4 && args[3]->getType() != DT_VOID) {
       if(args[3]->getType() != DT_STRING || args[3]->getForm() != DF_SCALAR) {
           throw IllegalArgumentException("odbc::query", "The 4th argument must be the name of encoding, e.g.(GB2312)");
       }
       if(Util::lower(args[3]->getString()) == "gb2312") {
            srcEncoding = Encoding::GB2312;
       } else {
           throw IllegalArgumentException("odbc::query", "The 4th argument must be the name of encoding, e.g.(GB2312)");
       }
    }
    ...
    vector<vector<U8>> buffers = vector<vector<U8>>(columns, vector<U8>(Util::BUF_SIZE));
                ...
                switch (columnTypes[col]) {
                    ...
                    case DT_TIMESTAMP: {
                        ...
                        if (memcmp(&ts, &fallback, sizeof(nanodbc::timestamp)) == 0) {
                            ...
                            // https://docs.microsoft.com/en-us/sql/odbc/reference/appendixes/c-data-types
                            buffers[col][curLine].longVal = Util::countDays(ts.year, ts.month, ts.day) * 86400000ll + ((ts.hour * 60 + ts.min) * 60 + ts.sec) * 1000 + ts.fract / 1000000;
                        }
                    } break;
                    case DT_SECOND: {
                    ...
                    case DT_NANOTIME: {
                        ...
                        if (memcmp(&ts, &fallback, sizeof(nanodbc::timestamp)) == 0) {
                            ...
                            // ts.fract is the number of billionths of a second.
                            // https://docs.microsoft.com/en-us/sql/odbc/reference/appendixes/c-data-types
                            buffers[col][curLine].longVal = ((ts.hour * 60 + ts.min) * 60 + ts.sec) * 1000000000ll + ts.fract;
                        }
                    } break;
                    case DT_NANOTIMESTAMP: {
                        nanodbc::timestamp fallback = {0, 0, 0, 0, 0, 0, 0};
                        ...
                        if (memcmp(&ts, &fallback, sizeof(nanodbc::timestamp)) == 0) {
                            ...
                            // ts.fract is the number of billionths of a second.
                            // https://docs.microsoft.com/en-us/sql/odbc/reference/appendixes/c-data-types
                            buffers[col][curLine].longVal = Util::countDays(ts.year, ts.month, ts.day) * 86400000000000ll + ((ts.hour * 60 + ts.min) * 60 + ts.sec) * 1000000000ll + ts.fract;
                        }
                    } break;
                    case DT_SYMBOL: {
                ...
                switch (rawType) {
                    case DT_BOOL:
                        if (sqltype == SQL_DECIMAL || sqltype == SQL_NUMERIC) {
                            for (int j = 0; j < curLine; ++j) charBuf[j] = (bool)colBuf[j].doubleVal;
                        } else {
                    ...
                    case DT_CHAR:
                        if (sqltype == SQL_DECIMAL || sqltype == SQL_NUMERIC) {
                            for (int j = 0; j < curLine; ++j) charBuf[j] = (char)colBuf[j].doubleVal;
                        } else {
                            for (int j = 0; j < curLine; ++j) charBuf[j] = colBuf[j].charVal;
                        }
                    ...
                    case DT_SHORT:
                        if (sqltype == SQL_DECIMAL || sqltype == SQL_NUMERIC) {
                            for (int j = 0; j < curLine; ++j) shortBuf[j] = (short)colBuf[j].doubleVal;
                        } else {
                    ...
                    case DT_INT:
                        if (sqltype == SQL_DECIMAL || sqltype == SQL_NUMERIC) {
                            for (int j = 0; j < curLine; ++j) buf[j].intVal = (int)colBuf[j].doubleVal;
                        } else {
                    ...
                    case DT_LONG:
                        if (sqltype == SQL_DECIMAL || sqltype == SQL_NUMERIC) {
                            for (int j = 0; j < curLine; ++j) colBuf[j].longVal = (long long)colBuf[j].doubleVal;
                        }
                        arrCol[col]->appendLong((long long*)(&colBuf[0]), curLine);
                        break;
                    case DT_FLOAT:
                        if (sqltype == SQL_DECIMAL || sqltype == SQL_NUMERIC) {
                            for (int j = 0; j < curLine; ++j) buf[j].floatVal = (long long)colBuf[j].doubleVal;
                        } else {
                    ...
                    case DT_STRING:
                        ...
                        if (sqltype == SQL_DECIMAL || sqltype == SQL_NUMERIC) {
                            for (int j = 0; j < curLine; ++j) {
                                char strBuf[120];
                                int nbytes = snprintf(strBuf, sizeof(strBuf), "%lf", colBuf[j].doubleVal);
                            ...
                            for (int j = 0; j < curLine; ++j) buf[j].pointer = colBuf[j].pointer;
                        }
