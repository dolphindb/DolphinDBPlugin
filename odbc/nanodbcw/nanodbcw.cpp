...
// Import string types defined in header file, so we don't have to type nanodbcw:: everywhere
using nanodbcw::wide_char_t;
using nanodbcw::wide_string;
...
recent_error(SQLHANDLE handle, SQLSMALLINT handle_type, long& native, std::string& state)
{
    ...
    do
    {
        ...
        result += nanodbcw::string(sql_message.begin(), sql_message.end());
        i++;
...
#define NANODBC_THROW_DATABASE_ERROR(handle, handle_type)                                          \
    throw nanodbcw::database_error(handle, handle_type, "ODBC database error: ") /**/
#else
#define NANODBC_THROW_DATABASE_ERROR(handle, handle_type)                                          \
    throw nanodbcw::database_error(                                                                 \
...
template <>
struct sql_ctype<nanodbcw::date>
{
    static const SQLSMALLINT value = SQL_C_DATE;
};

template <>
struct sql_ctype<nanodbcw::time>
{
    static const SQLSMALLINT value = SQL_C_TIME;
};

template <>
struct sql_ctype<nanodbcw::timestamp>
{
