...
* dataBaseType: 数据库类型。如"MySQL", "SQLServer", "PostgreSQL", "ClickHouse" 不区分大小写。建议连接时指定该参数，否则写入数据时可能出现报错。
* 注意避免数据库连接的端口设置与DolphinDB端口重复，会出现server crash的现象。
...
* batchSize:每当从odbc查询到的数据行数到达batchSize后，会将当前已经读到的数据append到表t中。默认值为262144。
...
| Clickhouse       | 需要unixodbc2.3.6及以上版本。<br />低版本unixodbc可能会导致在用户输入有语法错误的查询语句后程序崩溃。 |      |
| SQLite           | 不推荐使用                                              |      |
| Oracle           | 不推荐使用                                              |      |
