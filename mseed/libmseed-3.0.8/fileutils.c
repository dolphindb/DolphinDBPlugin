...
ms3_readmsr (MS3Record **ppmsr, const char *mspath, int64_t *fpos,
             int8_t *last, uint32_t flags, int8_t verbose,FILE *fp, MS3FileParam *dmsfp)
{

  //MS3FileParam *msfp = &gMS3FileParam;
  return ms3_readmsr_selection (&dmsfp, ppmsr, mspath, fpos, last,
                                flags, NULL, verbose,fp);
} /* End of ms3_readmsr() */
...
ms3_readmsr_r (MS3FileParam **ppmsfp, MS3Record **ppmsr, const char *mspath,
               int64_t *fpos, int8_t *last, uint32_t flags, int8_t verbose,FILE *fp)
{
...
ms3_readmsr_selection (MS3FileParam **ppmsfp, MS3Record **ppmsr, const char *mspath,
                       ...
                       MS3Selections *selections, int8_t verbose,FILE *fp)
{
  MS3FileParam *msfp;
...
//    }
  ...
  if (msfp->input.handle == NULL)
  {
    ...
    {
      if (msio_fopen(&msfp->input, msfp->path, "rb", &msfp->startoffset, &msfp->endoffset,fp))
      {
        msr3_free (ppmsr);
        return MS_GENERROR;
      }
                             ...
                             int8_t splitversion, uint32_t flags, int8_t verbose)
{
  ...
  while ((retcode = ms3_readmsr_selection (&msfp, &msr, mspath, &fpos, NULL,
                                           flags, selections, verbose,NULL)) == MS_NOERROR)
  {
  ...
  ms3_readmsr_selection (&msfp, &msr, NULL, NULL, NULL, 0, NULL, 0,NULL);
