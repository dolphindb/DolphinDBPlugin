...
OrderTracker<OrderPtr>::OrderTracker(const OrderPtr &order) : order_(order), open_qty_(order->order_qty()), reserved_(0), conditions_(0) {
...
template <class OrderPtr>
Quantity OrderTracker<OrderPtr>::reserve(int64_t reserved) {
    reserved_ += reserved;
    return open_qty_ - reserved_;
}

template <class OrderPtr>
void OrderTracker<OrderPtr>::change_qty(int64_t delta) {
