...
class OrderBook {
...
#ifdef TEST
  // used for tests only
  void price_change_test(Price price) {
 ...
 protected:
  ...
  Quantity create_trade(
    Tracker &inbound_tracker,
    Tracker &current_tracker,
    Quantity max_quantity = UINT64_MAX);
 ...
 private:
  ...
  TrackerMap trailingStopLossBids_;
  TrackerMap trailingStopLossAsks_;
...
OrderBook<OrderPtr>::OrderBook(const std::string &symbol)
  : symbol_(symbol),
    ...
    lastPeak_(MARKET_ORDER_PRICE),
    lastBottom_(MARKET_ORDER_PRICE)
{
...
OrderBook<OrderPtr>::set_market_price(Price price)
{
  ...
  marketPrice_ = price;
  if (oldMarketPrice == MARKET_ORDER_PRICE) {
    lastPeak_ = price;
    lastBottom_ = price;
  }
  if (price > oldMarketPrice || oldMarketPrice == MARKET_ORDER_PRICE) {
    ...
    // irrelevant with side, always true
    check_stop_loss_orders(true, lastPeak_ - lastBottom_, trailingStopLossBids_);
    ...
    // irrelevant with side, always true
    check_stop_loss_orders(true, lastPeak_ - lastBottom_, trailingStopLossAsks_);
  }
}
...
template<class OrderPtr>
void
OrderBook<OrderPtr>::cancel(uint64_t id)
{
  bool found = false;
  Quantity open_qty;
  auto it = orders_.find(id);
  if(it != orders_.end()) {
    TrackerMap &marketSideMap =
      it->second->is_buy() ? bids_ : asks_;
    TrackerMap &holderSideMap =
      it->second->is_buy() ?
      (it->second->stop_loss() ? stopLossBids_ : takeProfitBids_) :
      (it->second->take_profit() ? takeProfitAsks_ : stopLossAsks_);
  ...
  if (found) {
    ...
    callbacks_.push_back(TypedCallback::book_update(this));
    orders_.erase(it->second->order_id());
  } else {
...
OrderBook<OrderPtr>::add_stop_order(Tracker &tracker)
{
  ...
  } else if (tracker.ptr()->take_profit()) {
    isStopped = key > marketPrice_;
    if (isStopped) {
      if (isBuy) {
  ...
  } else if (tracker.ptr()->trailing_stop_loss()) {
    isStopped = true;
    if(isBuy) {
      trailingStopLossBids_.emplace(key, std::move(tracker));
    } else {
      trailingStopLossAsks_.emplace(key, std::move(tracker));
    }
  }
  return isStopped;
}
...
template<class OrderPtr>
void
OrderBook<OrderPtr>::check_stop_loss_orders(
  bool side,
  Price price,
  TrackerMap &stops)
{
...
template<class OrderPtr>
void
OrderBook<OrderPtr>::check_take_profit_orders(
  bool side,
  Price price,
  TrackerMap &stops) {
  ComparablePrice until(side, price);
...
OrderBook<OrderPtr>::submit_order(Tracker &inbound)
{
  ...
  if(inbound.ptr()->expired()) {
    orders_.erase(inbound.ptr()->order_id());
    callbacks_.push_back(
      TypedCallback::timeout(
        inbound.ptr(),
        inbound.open_qty()));
    return false;
  }
...
template<class OrderPtr>
bool
OrderBook<OrderPtr>::find_on_holders(
  const OrderPtr &order,
  TrackerMap &map,
  typename TrackerMap::iterator &result)
{
...
OrderBook<OrderPtr>::add_order(Tracker &inbound, Price order_price)
{
  ...
  if (inbound.open_qty() && !inbound.immediate_or_cancel()) {
    ...
    if (order->is_buy()) {
      ...
      asks_.insert(
        std::make_pair(
          ComparablePrice(false, order_price),
          inbound));
...
template<class OrderPtr>
bool
OrderBook<OrderPtr>::check_deferred_aons(
  DeferredMatches &aons,
  TrackerMap &deferredTrackers,
  TrackerMap &marketTrackers)
{
...
template<class OrderPtr>
bool
OrderBook<OrderPtr>::match_order(
  Tracker &inbound,
  Price inbound_price,
  TrackerMap &current_orders,
  DeferredMatches &deferred_aons)
{
  if (inbound.all_or_none()) {
    return match_aon_order(
      inbound,
      inbound_price,
      current_orders,
      deferred_aons);
  }
  return match_regular_order(
    inbound,
    inbound_price,
    current_orders,
    deferred_aons);
...
template<class OrderPtr>
bool
OrderBook<OrderPtr>::match_regular_order(
  Tracker &inbound,
  Price inbound_price,
  TrackerMap &current_orders,
  DeferredMatches &deferred_aons)
...
template<class OrderPtr>
bool
OrderBook<OrderPtr>::match_aon_order(
  Tracker &inbound,
  Price inbound_price,
  TrackerMap &current_orders,
  DeferredMatches &deferred_aons)
...
template<class OrderPtr>
Quantity
OrderBook<OrderPtr>::create_trade(
  Tracker &inbound_tracker,
  Tracker &current_tracker,
...
OrderBook<OrderPtr>::callback_now()
{
  ...
  if (!handling_callbacks_) {
    ...
    while (!callbacks_.empty()) {
      ...
      for (auto cb = workingCallbacks_.begin(); cb != workingCallbacks_.end();
           ++cb) {
        try {
          perform_callback(*cb);
        }
...
OrderBook<OrderPtr>::perform_callback(TypedCallback &cb)
{
  switch (cb.type) {
    case TypedCallback::cb_order_fill: {
      ...
      bool inbound_filled =
        (cb.flags
          & (TypedCallback::ff_inbound_filled | TypedCallback::ff_both_filled))
          != 0;
      bool matched_filled =
        (cb.flags
          & (TypedCallback::ff_matched_filled | TypedCallback::ff_both_filled))
          != 0;
      on_fill(
        cb.order, cb.matched_order,
        cb.quantity, fill_cost,
        inbound_filled,
        matched_filled);
      if (order_listener_) {
        order_listener_->on_fill(
          cb.order, cb.matched_order,
          cb.quantity, fill_cost);
      }
    ...
    case TypedCallback::cb_order_accept:on_accept(cb.order, cb.quantity);
      if (order_listener_) {
        order_listener_->on_accept(cb.order);
      }
      break;
    case TypedCallback::cb_order_reject:on_reject(cb.order, cb.reject_reason);
      if (order_listener_) {
        order_listener_->on_reject(cb.order, cb.reject_reason);
      }
      break;
    case TypedCallback::cb_order_timeout:on_timeout(cb.order, cb.quantity);
      if (order_listener_) {
        order_listener_->on_timeout(cb.order);
      }
      break;
    case TypedCallback::cb_order_cancel:on_cancel(cb.order, cb.quantity);
      if (order_listener_) {
    ...
    case TypedCallback::cb_order_replace_reject:
      ...
      break;
    case TypedCallback::cb_book_update:on_order_book_change();
