...
DepthOrderBook<OrderPtr>::DepthOrderBook(const std::string &symbol, int size)
...
template<class OrderPtr>
void
DepthOrderBook<OrderPtr>::set_bbo_listener(TypedBboListener *listener)
{
...
template<class OrderPtr>
void
DepthOrderBook<OrderPtr>::set_depth_listener(TypedDepthListener *listener)
{
...
template<class OrderPtr>
void
DepthOrderBook<OrderPtr>::on_accept(const OrderPtr &order, Quantity quantity)
{
...
template<class OrderPtr>
void
DepthOrderBook<OrderPtr>::on_fill(
  const OrderPtr &order,
  const OrderPtr &matched_order,
  Quantity quantity,
  Cost fill_cost,
...
template<class OrderPtr>
void
DepthOrderBook<OrderPtr>::on_cancel(const OrderPtr &order, Quantity quantity)
{
  // If the order is a limit order
  if (order->is_limit()) {
...
template<class OrderPtr>
void
DepthOrderBook<OrderPtr>::on_replace(
  const OrderPtr &order,
  Quantity current_qty,
  Quantity new_qty,
  Price new_price)
...
template<class OrderPtr>
void
DepthOrderBook<OrderPtr>::on_order_book_change()
{
...
template<class OrderPtr>
inline typename DepthOrderBook<OrderPtr>::DepthTracker &
DepthOrderBook<OrderPtr>::depth()
{
  return depth_;
}

template<class OrderPtr>
inline const typename DepthOrderBook<OrderPtr>::DepthTracker &
DepthOrderBook<OrderPtr>::depth() const
{
  return depth_;
}
