...
using OrderQueue = SynchronizedQueue<OrderPtr>;
using OrderQueuePtr = std::shared_ptr<OrderQueue>;

class Exchange;
class ExchangeExecutor : public Runnable {
 public:
  ExchangeExecutor(Exchange *exchange);
 protected:
  void run() override;
 private:
  Exchange *exchange_;
...
class Exchange :
 ...
 public:
  ...
  void on_cancel_reject(const OrderPtr &order, const char *reason) override
  {
    ...
    //debug() << "order cancel rejected #" << order->order_id() << std::endl;
    outputToBuffer(order);
  }
  ...
  void on_replace_reject(const OrderPtr &order, const char *reason) override
  {
    ...
    //debug() << "order replace rejected #" << order->order_id() << std::endl;
    outputToBuffer(order);
  }

  void on_trade(const OrderBook *book, Quantity qty, Cost cost) override
  {
    //debug() << "trade " << qty << " " << book->symbol() << " cost " << cost
    //        << std::endl;
  }
