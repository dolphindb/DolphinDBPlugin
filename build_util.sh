...
function detect_cmake_path() {
    ...
    libs=(
        absl arrow aws blosc boost cppkafka curl fftw hiredis jsoncpp krb5 lgbm libarchive libevent librdkafka
        libtorch libzmq lz4 mariadb open62541 openldap paho protobuf pulsar quantlib rocketmq sasl2 snappy thrift
        unixODBC wavelib xgboost xsimd zlib zstd
    )
    for lib in "${libs[@]}"; do
    ...
    # arrow 9.0.0 failed to find it's parquet, so we need to set Parquet_DIR manually
    if [[ "$CC" == "aarch64-linux-gnu-gcc" ]]; then
        cmake_path="-DParquet_DIR=$(ls -d $toolchain_dir/arrow-*)/lib/cmake/arrow $cmake_path"
    else
        cmake_path="-DParquet_DIR=$(ls -d $toolchain_dir/arrow-*)/lib64/cmake/arrow $cmake_path"
    fi
    set -e
}
