...
ConstantSP packDataTypeEnum(Type type) {
    return new Int(_decimal_util::packDecimalTypeAndScale((DATA_TYPE)type.first, EXPARM_VALUE(type.second)));
}
...
Vector* createArrayVector(Type type, INDEX size, INDEX valueSize, INDEX capacity, INDEX valueCapacity) {
    ...
    return InternalUtil::createArrayVector((DATA_TYPE)type.first, size, valueSize, capacity, valueCapacity, true, EXPARM_VALUE(type.second));
}
