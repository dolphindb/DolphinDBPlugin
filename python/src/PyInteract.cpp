...
PythonClassSP PythonClass::global_class;
...
using ddb::String;
using ddb::Void;
using ddb::PythonClass;
using ddb::PythonInstance;
using ddb::PythonInstanceSP;
using ddb::VectorSP;

ConstantSP initialize(Heap *heap, std::vector<ConstantSP> &arguments) {
    std::ignore = heap;
    std::ignore = arguments;
    PythonClass::global_class = PythonClass::createPyClass("builtin", "Object");
    ddb::PythonInitial::GetInstance();
    return ddb::Util::createConstant(ddb::DT_VOID);
}
...
ConstantSP pyNone(Heap *heap, std::vector<ConstantSP> &arguments) {
    ...
    const py::gil_scoped_acquire acquire;
    return new PythonInstance(py::none(), PythonClass::global_class);
}
...
ConstantSP pyInt(Heap *heap, std::vector<ConstantSP> &arguments) {
    ...
    const py::gil_scoped_acquire acquire;
    return new PythonInstance(py::int_(value->getLong()), PythonClass::global_class);
}
...
ConstantSP pyFloat(Heap *heap, std::vector<ConstantSP> &arguments) {
    ...
    const py::gil_scoped_acquire acquire;
    return new PythonInstance(py::float_(value->getDouble()), PythonClass::global_class);
}
...
ConstantSP pyBool(Heap *heap, std::vector<ConstantSP> &arguments) {
    ...
    const py::gil_scoped_acquire acquire;
    return new PythonInstance(py::bool_(value->getBool() != 0), PythonClass::global_class);
}
...
ConstantSP pyStr(Heap *heap, std::vector<ConstantSP> &arguments) {
    ...
    return new PythonInstance(py::reinterpret_steal<py::object>(tmp), PythonClass::global_class);
}
...
ConstantSP pyBytes(Heap *heap, std::vector<ConstantSP> &arguments) {
    ...
    const py::gil_scoped_acquire acquire;
    return new PythonInstance(py::bytes(str.data(), str.size()), PythonClass::global_class);
}
...
ConstantSP toPy(Heap *heap, std::vector<ConstantSP> &arguments) {
    ...
    return new PythonInstance(res, PythonClass::global_class);
}
...
ConstantSP toNumpy(Heap *heap, std::vector<ConstantSP> &arguments) {
    ...
    if (form != ddb::DF_VECTOR && form != ddb::DF_MATRIX) {
...
ConstantSP fromNumpy(Heap *heap, std::vector<ConstantSP> &arguments) {
    std::ignore = heap;
    const auto &data = arguments[0];
    if (data->isNull()) {
        throw IllegalArgumentException(__func__, "Input is not python object");
...
ConstantSP toPandas(Heap *heap, std::vector<ConstantSP> &arguments) {
    ...
    const auto &data = arguments[0];
    const auto form = data->getForm();
    if (form != ddb::DF_VECTOR && form != ddb::DF_TABLE) {
        throw ddb::IllegalArgumentException(__func__, "[data] must be a vector or table.");
    }
...
ConstantSP importModule(Heap *heap, std::vector<ConstantSP> &arguments) {
    std::ignore = heap;
    if (arguments[0]->getType() != ddb::DT_STRING) {
    ...
    try {
        ...
        throw ddb::RuntimeException(std::string("Failed to import module ") + name + ": " + e.what());
        return new Void();
    }

    LOG_INFO("Imported ", name);
    return new PythonInstance(mod, PythonClass::global_class);
}
...
ConstantSP pyContainer(const ConstantSP &data)
{
    ...
    return new PythonInstance(T(res), PythonClass::global_class);
}
...
ConstantSP pyDict(Heap *heap, std::vector<ConstantSP> &args) {
    ...
    return new PythonInstance(py_dict, PythonClass::global_class);
}
...
ConstantSP setGlobalConfig(Heap *heap, std::vector<ConstantSP> &arguments) {
    std::ignore = heap;
    if (!arguments[0]->isScalar() || arguments[0]->getType() != ddb::DT_STRING) {
