    ...
    int strategy, const char* password, uLong crcForCrypting, uLong versionMadeBy, uLong flagBase, int zip64)
{
    ...
    zip64local_putValue_inmemory(zi->ci.central_header+4, (uLong)798, 2);
    ...
    if ((err == Z_OK) && ((zi->ci.flag & 1) != 0))
    {
        ...
        if (zi->ci.method == AES_METHOD)
        {
            ...
            if(fcrypt_init(AES_ENCRYPTIONMODE, password, strlen(password), saltvalue, passverify, &zi->ci.aes_ctx))
                return ZIP_ERRNO;
