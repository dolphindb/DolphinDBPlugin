...
ConstantSP nsqGetSchema(Heap *heap, vector<ConstantSP> &args) {
    ...
    auto [data, market] = get_types(dataType, "sh");
    return g_nsq->getSchema(data);
}
...
ConstantSP nsqSubscribe(Heap *heap, vector<ConstantSP> &args) {
    ...
    auto [data, market] = get_types(data_str, market_str);
...
ConstantSP nsqUnsubscribe(Heap *heap, vector<ConstantSP> &args) {
    ...
    auto [data, market] = get_types(dataType, marketType);
    /// unsubscribe
    g_nsq->unsubscribe(data, market);
    return new Void();
}
