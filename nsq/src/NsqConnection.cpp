...
const char* market_id(nsq_market market) {
    switch (market) {
...
void NsqConnection::subscribeTradeEntrust(Heap *heap, nsq_market market, const DictionarySP &tableDict, long long queueDepth, const vector<string> &codes) {
...
void NsqConnection::unsubscribe(std::pair<nsq_data, nsq_market> stream)
{
    auto types = g_spi->queues_->getTypesToCancel(stream.first, stream.second);
    for (const auto &type : types.first) {
        subscribeOrCancel(type, stream.second, true);
    }
    for (const auto &type: types.second) {
        g_spi->queues_->stop(type, stream.second);
    }
}
