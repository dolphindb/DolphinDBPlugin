...
void NsqConnection::connect(const string &configFilePath) {
    loginErrMsg_ = "";
    ...
    if (!loginErrMsg_.empty()) {
        throw RuntimeException(NSQ_PREFIX + "establish connection failed. " +  loginErrMsg_);
    }
...
void NsqConnection::initInstance(const string &configFilePath, const DictionarySP& options, const string &username, const string &password, const string &version) {
    ...
    if (instancePtr == nullptr) {
        ...
        instancePtr->spi_->username_ = username;
        instancePtr->spi_->password_ = password;
    ...
    try {
        ...
        instancePtr->connect(configFilePath);
...
void NsqConnection::unsubscribe(const string &dataType, const string &marketType) {
    ...
    std::tie(typesForCancel, typesForStop) = spi_->queues_->getTypesToCancel(dataType, marketType);
