...
private:
    ...
    void addThreadedQueue(Heap *heap, nsq_data data, nsq_market market, const TableSP &table, MetaTable meta,
                          unordered_map<nsq_market, SmartPointer<ThreadedQueue<DataStruct>>, enum_hash> &map,
    ...
    unordered_map<nsq_market, SmartPointer<ThreadedQueue<nsqUtil::TradeDataStruct>>, enum_hash> threadedQueueMapT_;
    unordered_map<nsq_market, SmartPointer<ThreadedQueue<nsqUtil::EntrustDataStruct>>, enum_hash> threadedQueueMapE_;
    unordered_map<nsq_market, SmartPointer<ThreadedQueue<nsqUtil::SnapshotDataStruct>>, enum_hash> threadedQueueMapS_;
    unordered_map<nsq_market, unordered_map<int, SmartPointer<ThreadedQueue<nsqUtil::TradeEntrustDataStruct>>>, enum_hash>
        threadedQueueMapTE_;
};
...
void runWithWorkers(long long totalRows, long long workerCount, Func &&func)
{
    ...
    for (long long i = 0; i < workerCount; ++i) {
        ...
        ThreadSP worker = new Thread(new ddb::Executor([&, i, start, end]() {
            func(i, start, end);
        }));
        worker->start();
                                         ...
                                         long long batchCount, long long queueDepth, long long threadCount)
{
    return runBatchCase(test, dataType, batchSize, batchCount, queueDepth, threadCount,
                        [&](long long expectedRows, const Clock::time_point &start) {
                            std::vector<TableSP> tables;
                            tables.reserve(static_cast<size_t>(threadCount));
                            unordered_map<nsq_market, SmartPointer<ThreadedQueue<nsqUtil::TradeDataStruct>>, enum_hash> queues;
                            RouteMap routes;
                            for (long long i = 0; i < threadCount; ++i) {
                                long long rows = expectedRows * (i + 1) / threadCount - expectedRows * i / threadCount;
                                tables.push_back(createOutputTable(nsqUtil::tradeMeta, rows));
                                           ...
                                           long long batchCount, long long queueDepth, long long threadCount)
{
    return runBatchCase(test, dataType, batchSize, batchCount, queueDepth, threadCount,
                        [&](long long expectedRows, const Clock::time_point &start) {
                            std::vector<TableSP> tables;
                            tables.reserve(static_cast<size_t>(threadCount));
                            unordered_map<nsq_market, SmartPointer<ThreadedQueue<nsqUtil::EntrustDataStruct>>, enum_hash> queues;
                            RouteMap routes;
                            for (long long i = 0; i < threadCount; ++i) {
                                long long rows = expectedRows * (i + 1) / threadCount - expectedRows * i / threadCount;
                                tables.push_back(createOutputTable(nsqUtil::entrustMeta, rows));
                                            ...
                                            long long batchCount, long long queueDepth, long long threadCount)
{
    return runBatchCase(test, dataType, batchSize, batchCount, queueDepth, threadCount,
                        [&](long long expectedRows, const Clock::time_point &start) {
                            ...
                            unordered_map<nsq_market, SmartPointer<ThreadedQueue<nsqUtil::SnapshotDataStruct>>, enum_hash> queues;
                            RouteMap routes;
                            for (long long i = 0; i < threadCount; ++i) {
                                long long rows = expectedRows * (i + 1) / threadCount - expectedRows * i / threadCount;
                                tables.push_back(createOutputTable(nsqUtil::snapshotMetaConcise, rows));
