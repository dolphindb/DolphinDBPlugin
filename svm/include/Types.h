...
#define CONSTANT_DOUBLE 14

#define CONSTANT_SYMBOL 15
#define CONSTANT_STRING 16
#define CONSTANT_DOUBLE_ENUM 17
...
const size_t MAX_ARRAY_BUFFER = 1024000;
...
enum DATA_TYPE {DT_VOID,DT_BOOL,DT_CHAR,DT_SHORT,DT_INT,DT_LONG,DT_DATE,DT_MONTH,DT_TIME,DT_MINUTE,DT_SECOND,DT_DATETIME,
...
/**
 * The order of the enumerated DATA_FORM items matters. In Constant class, we use 3 bits rather than a enum variable to
...
enum STATEMENT_TYPE {IF,DO,FOR,ASSIGN,ATTRASSIGN,MULTIASSIGN,GLOBALASSIGN,PROCEDURE,RETURN,CONTINUE,BREAK,EMPTY,ADHOC,SHOWTIME,INSERT,UPDATE,ASSERT,ANNOTATE,TRYCATCH,THROW,MODULE,USE,SHARE};

enum OBJECT_TYPE {CONSTOBJ,VAR,GLOBAL,ATTR,DIM,TUPLE,FUNCTION,EXPRESSION,COLUMN,COLUMNDEF,SQLQUERY,TABLEJOINER,VIRTUALCONST,MAPPEDCOL};

enum TABLE_TYPE {BASICTBL,FILETBL,JOINTBL,SEGTBL};

enum IO_ERR {OK,DISCONNECTED,NODATA,NOSPACE,TOO_LARGE_DATA,INPROGRESS,INVALIDDATA,END_OF_STREAM,READONLY,WRITEONLY,NOTEXIST,OTHERERR};

enum CONSOLE_TYPE {STD,HTTP,TERMINAL,API,API2,JSON,UNSUPPORTED};
...
enum OUTPUT_TYPE {STDOUT, HTTPOUT, APIOUT};
