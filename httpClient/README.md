...
需要构建 libcurl, libssl（版本为1.0.2）, libcrypto（版本为1.0.2）和 libz 的静态链接库。
...
### 2.1 httpClient::httpGet(url,[params],[timeout],[headers])
...
### 2.2 httpClient::httpPost(url,[params],[timeout],[headers])
...
```
### 2.3 httpClient::sendEmail(userId,pwd,recipient,subject,body)

发送邮件。
- 使用本插件的邮件发送函数通常需要在邮件服务商上面设置开启smtp协议，还需要获取邮箱密码，参数pwd为邮箱密码的字符串。
