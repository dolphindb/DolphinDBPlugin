...
ConstantSP httpGet(Heap *heap, vector<ConstantSP> &args) {
  ...
  if (args.size() >= 2 && !args[1]->isNull()) {
    ...
    if ((params->getType() != DT_STRING || params->getForm() != DF_SCALAR) &&
      ...
      throw IllegalArgumentException(__FUNCTION__,
                                     "params must be a string or a dictionary "
                                     "with STRING-STRING key-value type");
  } else
    params = new String("");
  if (args.size() >= 3) {
    timeout = args[2];
  ...
  if (args.size() >= 4) {
    nobody = args[3];
    if (nobody->getType() != DT_BOOL)
      throw IllegalArgumentException(__FUNCTION__, "nobody must be an bool");
  } else
    nobody = new Bool(0);

  return httpClient::httpRequest(httpClient::GET, url, params, timeout, nobody);
}
...
ConstantSP httpRequest(RequestMethod method, const ConstantSP &url,
  ...
  if (curl) {
    ...
    if (nobody->getBool()) curl_easy_setopt(curl, CURLOPT_NOBODY, 1L);
