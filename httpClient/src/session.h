...
public:
    WriteData(HttpType flag, int parserInterval, ConstantSP handle, FunctionDefSP parser, const SessionSP &session,
              bool needStop, HttpSession *http) :
            flag_(flag), parserInterval_(parserInterval), handle_(handle), parser_(parser), session_(session),
            needStop_(needStop), http_(http) {}
...
public:
    ...
    HttpSession(std::string startUrl, std::string endUrl, string cookieSet, SessionSP session) :
            data_(HttpData, 256, nullptr, nullptr, nullptr, false, this),
            head_(HttpHead, 256, nullptr, nullptr, nullptr, false, nullptr), isStopped_(1), cookieSet_(cookieSet) {
        startUrl_ = startUrl;
        endUrl_ = endUrl;
        ...
        if (cookieSet != "") {
            ...
            if (file != NULL) {
                ...
                LOG_ERR("Can't open the file " + cookieSet);
            }
        } else {
            ...
            if (ret->getType() != DT_STRING || ret->getForm() != DF_SCALAR) {
                LOG_ERR("getHomeDir must be return a string scalar");
            } else {
                ...
                if (file != NULL) {
                    ...
                    LOG_ERR("Can't open the file " + dir);
                }
            }
        }
    }
    ...
    void setCookie(string param, SessionSP session) {
        ...
        if (cookieSet_ != "") {
            ...
            if (file != nullptr) {
                ...
                LOG_ERR("Can't open the file " + cookieSet_);
            }
        } else {
            vector<ConstantSP> args;
            ConstantSP ret = session->getFunctionDef("getHomeDir")->call(session->getHeap().get(), args);
            if (ret->getType() != DT_STRING || ret->getForm() != DF_SCALAR) {
                ...
                if (file != nullptr) {
                    ...
                    LOG_ERR("Can't write to the file " + dir);
                }
            }
        }
    }
