...
size_t parserWriteData(void *ptr, size_t size, size_t nmemb, WriteData *data) {

    size_t writeSize = size * nmemb;
    data->buffer_.append((char *) ptr, writeSize);
    long long start = 0;
    size_t end = 0;
    if (data->flag_ == HttpData && !data->parser_.isNull()) {
        ...
        while ((data->buffer_.size() - end) > mIndex) {
            ...
            try {
                ...
                args.push_back(new String(data->buffer_.substr(start, end - start)));
                ...
                if (parser_result->isTable()) {
                    ...
                    if (digital->isNull() || !(digital->getType() != DT_INT || digital->getType() != DT_LONG) ||
                        ...
                        LOG_ERR("The parser must return a dictionary with STRING type keys and ANY type values. One of the keys must be \"size\" and the corresponding value must be a LONG or INT type scalar. ");
                    } else {
                    ...
                    if (parser_data->isNull() || !parser_data->isTable()) {
                        LOG_ERR("The parser must return a dictionary with STRING type keys and ANY type values. One of the keys must be \"data\" and the corresponding value must be a table. ");
                    }
                } else {
                    LOG_ERR("The parser must return a dictionary with STRING type keys and ANY type values or a table. ");
                }
                if (!parser_data.isNull() && parser_data->isTable()) {
...
HttpSession::httpRequest(SessionSP &session, httpClient::RequestMethod method, const ConstantSP &handle,
    ...
    if (curl) {
        ...
        head_ = WriteData(HttpHead, parserInterval, nullptr, nullptr, nullptr, head_.needStop_, this);
        data_ = WriteData(HttpData, parserInterval, handle, parser, session, data_.needStop_, this);
        ...
        if (responseCode != 200)
            LOG_ERR(head_.buffer_ + data_.buffer_);
        curl_slist_free_all(headerList);
    } else {
