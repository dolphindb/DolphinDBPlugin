    ...
    for (int i = 0; i < nRows; ++i) {
        ...
        for (auto it = row.begin(); it != row.end(); ++it) {
            ...
            switch (dt[curCol]) {
                ...
                case DT_MONTH: {
                    static FunctionDefSP const date = Util::getFuncDefFromHeap(heap, "month");
                    ConstantSP d = date->call(heap, new String(it->get<string>()), void_);
                    cols[curCol]->setInt(i, d->getInt());
                    break;
                }
...
ConstantSP formatCsv(Heap* heap, vector<ConstantSP>& args) {
    ...
    if (args[3].isNull() || !args[3]->isTable()) {
        throw IllegalArgumentException("formatCsv(schema, delimiter=',', rowDelimiter=';', Table)",
                                       " the 4th argument must be a table");
    }
    TableSP t = args[3];
    ...
    for (int row = 0; row < nRows; ++row) {
        for (int col = 0; col < nCols; ++col) {
            ret.append(t->getColumn(col)->get(row)->getString());
...
ConstantSP createJsonFormatter(Heap* heap, vector<ConstantSP>& args) {
    static FunctionDefSP const formatter = Util::getFuncDefFromHeap(heap, "toStdJson");
    return formatter;
}
...
ConstantSP createCsvFormatter(Heap* heap, vector<ConstantSP>& args) {
    string usage = "createCsvFormatter(schema, [delimiter=','], [rowDelimiter=';']). ";
    char delimiter = ',';
    char rowDelimiter = ';';
    if (args.size() > 1) {
        if (args[1]->getType() != DT_CHAR || args[1]->getForm() != DF_SCALAR) {
