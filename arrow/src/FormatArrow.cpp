...
ConstantSP getConstantMarshal(Heap *heap, vector<ConstantSP> &args) {
    ...
    return Util::createResource((long long) marshalMap, "arrow marshal", onDestroy, heap);
}
...
ConstantSP getConstantUnmarshal(Heap *heap, vector<ConstantSP> &args) {
    ...
    return Util::createResource((long long) &unmarshalMap, "arrow unmarshal", onDestroy, heap);
}
