...
ConstantSP HdfsConnection::readFile(Heap *heap, HdfsConnectionSP conn, const string &path, FunctionDefSP handler) {
    ...
    handlerArgs[0] =
        Util::createResource((long long)buffer, "hdfs readFile address", onCloseChar, heap->currentSession());
    handlerArgs[1] =
        Util::createResource((long long)bufSize, "hdfs readFile length", onCloseLong, heap->currentSession());
