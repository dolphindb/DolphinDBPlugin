...
static int countLifeTime(const string& lifeTime) {
    ...
    for(size_t i = 0; i < lifeTime.size(); ++i) {
        ...
        if(c == '+') {
            if(current != -1) {
                throw RuntimeException(PLUGIN_HDFS_LOG_PREFIX + "bad lifeTime value " + lifeTime + ".");
            }
            sign = 1;
        } else if(c == '-') {
            if(current != -1) {
...
static bool hdfsKInit(
    ...
    try {
        ...
        if ((ret = krb5_init_context(&ctx)))
            throw RuntimeException(PLUGIN_HDFS_LOG_PREFIX + "cannot initialize context");
        ...
        if ((ret = krb5_kt_resolve(ctx, keytabFile.c_str(), &keytab)))
            throw RuntimeException(PLUGIN_HDFS_LOG_PREFIX + "cannot resolve keytab");
        ...
        if ((ret = krb5_cc_resolve(ctx, cacheFile.c_str(), &cache)))
            throw RuntimeException(PLUGIN_HDFS_LOG_PREFIX + "cannot open/initialize kerberos cache");
        Defer deferCache([&]() {if (cache) {krb5_cc_close(ctx, cache);}});

        // Convert a string principal name to a krb5_principal structure
        if ((ret = krb5_parse_name(ctx, principal.c_str(), &krb5Principal)))
            throw RuntimeException(PLUGIN_HDFS_LOG_PREFIX + "cannot parse principal string");
        ...
        if (ret) {
            throw RuntimeException(PLUGIN_HDFS_LOG_PREFIX + "cannot initialize credential options");
        }
        ...
        if ((ret = krb5_get_init_creds_keytab(ctx, &creds, krb5Principal, keytab, 0, NULL, options))) {
            throw RuntimeException(PLUGIN_HDFS_LOG_PREFIX + "cannot initialize keytab credentials [" + keytabFile
                + "] with err code " + to_string(ret) + ".");
        }
        // Defer deferCreds([&]() {krb5_free_creds(ctx, &creds);});

        // Initialize a credential cache
        if ((ret = krb5_cc_initialize(ctx, cache, krb5Principal))) {
            throw RuntimeException(PLUGIN_HDFS_LOG_PREFIX + "cannot initialize cache [" + cacheFile + "].");
        }

        // Store credentials in a credential cache
        if ((ret = krb5_cc_store_cred(ctx, cache, &creds))) {
            throw RuntimeException(PLUGIN_HDFS_LOG_PREFIX + "cannot store credentials[" + cacheFile + "].");
        }
        return success;
    } catch (RuntimeException &ex) {
        throw ex;
    } catch (std::exception &ex) {
        throw RuntimeException(PLUGIN_HDFS_LOG_PREFIX + ex.what());
    }
}
