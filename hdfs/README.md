...
* 修复对空文件夹调用 getListDirectory 时报错的问题。（**1.30.22**）
* 修复使用 readFile 循环读取文件时，由于未关闭文件句柄导致打开的文件句柄过多，进而导致 hadoop 数据节点不可用、连接断开无法继续读取文件的问题。（**1.30.22**）
* 修复了由于 connect 底层共用 hdfs 连接导致的函数行为混乱的问题。（**1.30.22**）

# 功能优化

* connect 函数新增支持配置集群环境。（**1.30.22**）
* 优化部分报错信息。（**1.30.22**）
