...
ConstantSP kdb::toDDB::fromDict(
    ...
    DictionarySP dict = Util::createDictionary(KDB_DDB_TYPE_MAP[keyType], NULL, KDB_DDB_TYPE_MAP[valueType], NULL);
...
ConstantSP kdb::toDDB::fromScalar(
    ...
    switch(-type) {
        ...
        case K_GUID: return new Uuid(data->u);
...
DatabaseUpdater::DatabaseUpdater(Heap *heap, SystemHandleSP dbHandle, ConstantSP tableName, ConstantSP partitionColumns, ConstantSP sortColumns, TableSP schema, FunctionDefSP transform): heap_(heap), dbHandle_(dbHandle), tableName_(tableName), partitionColumns_(partitionColumns), sortColumns_(sortColumns), schema_(schema), transform_(transform){
...
void DatabaseUpdater::append(TableSP table) {
    ...
    if (existsTable) {
        ...
        result = loadTableFunc_->call(heap_, loadTableArgs);
    } else {
        ConstantSP dummyTable;
        if (schema_.isNull() || schema_->isNull()) {
            // if schema is null, use input to form schema
            dummyTable = appendData->getInstance(0);
        } else {
            dummyTable = DBFileIO::createEmptyTableFromSchema(schema_);
        }

        vector<ConstantSP> createTableArgs = {dbHandle_, dummyTable, tableName_, partitionColumns_};

        if (!sortColumns_.isNull() && !sortColumns_->isNull()) {
            // TSDB sort columns
            createTableArgs.push_back(new Void());
            createTableArgs.push_back(sortColumns_);
        }
        result = createPartitionedTableFunc_->call(heap_, createTableArgs);
    }

    // do append
    vector<ConstantSP> appendArgs{result, appendData};
