...
* 新增支持读取 kdb+ char 型 nested list。（**1.30.22**）

# 故障修复

* 修复调用 loadTable 时不指定参数 *symPath* 导致 server 宕机的问题。（**1.30.22**）
* 修复 loadTable 无法正确解析用户自定义 sym 文件名的问题（**1.30.22**）
* 修复在关闭插件 kdb+的进程后，再次执行 loadTable 导致 server 宕机的问题。（**1.30.22**）
