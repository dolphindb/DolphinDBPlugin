...
- isFramed: 布尔值，可选。若不指定，则自动尝试 `TBufferedTransport` 和 `TFramedTransport`；若设置为 false，则仅尝试 `TBufferedTransport`；若设置为 true，则仅尝试 `TFramedTransport`。
- timeout: 建立连接（ConnTimeout）与接收回复（RecvTimeout）的最长等待时间，单位为毫秒，默认为5000ms，类型为 INT。

#### 详情

通过 Thrift server 与 HBase 建立一个连接，返回一个 HBase 连接的句柄。插件会自动尝试 `TBinaryProtocol` 和 `TCompactProtocol`。
...
- hbaseConnection: 通过 hbase::connect 获得的 HBase 句柄。
- tableName: 需要读取数据的表的名字，类型为 STRING。
- schema: 包含列名和列的数据类型的表。HBase 中数据以字节形式存储，默认返回的表中列类型为 STRING。可通过该参数指定列类型。
- scanOptions: 可选字典，用于透传 HBase Thrift `TScan` 参数。目前支持以下 key：
  - `startRow`(STRING scalar)
  - `stopRow`(STRING scalar)
  - `columns`(STRING scalar/vector)
  - `caching`(positive INT scalar) 配置含义见 [MapReduce Scan Caching](https://hbase.apache.org/docs/mapreduce#mapreduce-scan-caching)
  - `filterString`(STRING scalar) 配置含义见 [Filter Language](https://hbase.apache.org/docs/thrift-filter-language)
