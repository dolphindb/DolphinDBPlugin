...
ConstantSP createJsonDecoder(Heap* heap, vector<ConstantSP>& arguments)
{
    ...
    if (arguments.size() == 2 || arguments[2]->isNull()) {
        ...
        // std::unique_ptr<CoderImpl> decoder(new CoderImpl(heap->currentSession(), parser));
        // decoderRes = new CoderResource((long long)decoder.release(), "coder instance", onClose, heap->currentSession(), true);
    } else {
        decoderRes = new CoderImpl(heap->currentSession(), parser, handler, dummyTable, workerNum, batchSize, throttle);
        // std::unique_ptr<CoderImpl> decoder(new CoderImpl(heap->currentSession(), parser, handler, dummyTable, workerNum, batchSize, throttle));
        // decoderRes = new CoderResource((long long)decoder.release(), "coder instance", onClose, heap->currentSession(), true);
    }
