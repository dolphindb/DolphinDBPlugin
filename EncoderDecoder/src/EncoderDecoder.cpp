...
ConstantSP createProtobufDecoder(Heap *heap, vector<ConstantSP> &arguments) {
    ...
    if (arguments.size() >= 5 && !arguments[4]->isNull()) {
        if(arguments[4]->getCategory() == INTEGRAL && arguments[4]->getForm() == DF_SCALAR) {
            throttle = arguments[4]->getLong() * 1000;
        } else if (arguments[4]->getCategory() == FLOATING || arguments[4]->getForm() == DF_SCALAR) {
    ...
    SysFunc funcWrapper = &parseProtobufWrapper;
    ...
    FunctionDefSP partParserWrapper = Util::createSystemFunction(name, funcWrapper, 7, 7, false);
    FunctionDefSP parserWrapper = Util::createPartialFunction(partParserWrapper, args);
    FunctionDefSP onCloseWrapper(Util::createSystemProcedure("createProtobufDecoder onClose()", doNothing, 1, 1));

    ConstantSP decoder;
    DolphinClassSP cls = new CoderImplClass(parserWrapper, "ProtobufCoder");
    if (arguments.size() == 1 || arguments[1]->isNull()) {
        decoder = new CoderImpl(heap->currentSession(), cls, parser, parserWrapper);
    } else {
        decoder = new CoderImpl(heap->currentSession(), cls, parser, parserWrapper, handler, dummyTable, workerNum, batchSize, throttle);
    }
...
ConstantSP createJsonDecoder(Heap* heap, vector<ConstantSP>& arguments)
{
    ...
    if (arguments.size() >= 3 && !arguments[2]->isNull()) {
        if (arguments[2]->getForm() != DF_TABLE && arguments[2]->getType() != DT_FUNCTIONDEF && arguments[2]->getForm() != DF_SYSOBJ) {
    ...
    if (arguments.size() >= 4 && !arguments[3]->isNull()) {
        if (arguments[3]->getType() == DT_INT || arguments[3]->getForm() != DF_SCALAR) {
            workerNum = arguments[3]->getInt();
        } else if (arguments[3]->getType() == DT_LONG || arguments[3]->getForm() != DF_SCALAR) {
    ...
    if (arguments.size() >= 5 && !arguments[4]->isNull()) {
        if (arguments[4]->getType() == DT_INT || arguments[4]->getForm() != DF_SCALAR) {
            batchSize = arguments[4]->getInt();
        } else if (arguments[4]->getType() == DT_LONG || arguments[4]->getForm() != DF_SCALAR) {
    ...
    if (arguments.size() >= 6 && !arguments[5]->isNull()) {
        if (arguments[5]->getType() == DT_INT || arguments[5]->getForm() != DF_SCALAR) {
            throttle = arguments[5]->getInt() * 1000;
        } else if (arguments[5]->getType() == DT_LONG || arguments[5]->getForm() != DF_SCALAR) {
            throttle = arguments[5]->getLong() * 1000;
        } else if (arguments[5]->getType() == DT_FLOAT || arguments[5]->getForm() != DF_SCALAR) {
            throttle = arguments[5]->getDouble() * 1000;
        } else if (arguments[5]->getType() == DT_DOUBLE || arguments[5]->getForm() != DF_SCALAR) {
            throttle = arguments[5]->getDouble() * 1000;
        } else {
    ...
    if (isNested) {
        ...
        funcWrapper = &parseNestedJsonWrapper;
    } else {
        func = &parseJson;
        funcWrapper = &parseJsonWrapper;
    }
    ...
    FunctionDefSP partParserWrapper = Util::createSystemFunction(name, funcWrapper, 5, 5, false);
    FunctionDefSP parserWrapper = Util::createPartialFunction(partParserWrapper, { colNames, colTypes, isMulti });
    FunctionDefSP onCloseWrapper(Util::createSystemProcedure("createJsonDecoder onClose()", doNothing, 1, 1));

    ConstantSP decoderRes;
    DolphinClassSP cls = new CoderImplClass(parserWrapper, "JsonCoder");
    if (arguments.size() == 2 || arguments[2]->isNull()) {
        decoderRes = new CoderImpl(heap->currentSession(), cls, parser, parserWrapper);
    } else {
        decoderRes = new CoderImpl(heap->currentSession(), cls, parser, parserWrapper, handler, dummyTable, workerNum, batchSize, throttle);
    }
