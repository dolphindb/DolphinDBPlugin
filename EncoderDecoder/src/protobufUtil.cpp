...
ConstantSP parseProtobuf(Heap *heap, vector<ConstantSP> &args) {
    ...
    if (schemaPath == "dolphindb") {
        return parseProtobufDDB(heap, vec);
    } else if (Util::exists(schemaPath)) {
...
ConstantSP formatProtobuf(Heap *heap, vector<ConstantSP> &args) {
    ...
    if (args[0]->getString() == "dolphindb") {
        return formatProtobufDDB(mode, args[2]);
    } else {
        return formatProtobufDynamic(schemaName, mode, args[2]);
    }
    return new Void();
}
...
//========================================================================
// dolphindbFast.proto
// parse & format function for a faster dolphindb protobuf schema
// support dolphindb table, vector, scalar
//========================================================================

#include "protobuf/dolphindbFast.pb.h"
...
static ddbprotobuf::Scalar protobufSerialize(const ConstantSP &ddbScalar) {
    ...
    switch (ddbScalar->getType()) {
        ...
        case DT_SYMBOL:
            ...
            break;
        default:
            throw RuntimeException(ENCODERDECODER_PREFIX + "can not serialize " +
                                   Util::getDataTypeString(ddbScalar->getType()) + " type");
    }
    return pbScalar;
}
...
static ddbprotobuf::Vector protobufSerialize(const VectorSP &ddbVector) {
    ...
    pbVector.set_is_null(ddbVector->isNull());
    switch (ddbVector->getType()) {
        case DT_BOOL: {
            ddbprotobuf::boolVector *innerVector = new ddbprotobuf::boolVector();
            int size = ddbVector->size();
            vector<char> dataVec(size);
            ddbVector->getBool(0, size, dataVec.data());
            for (int i = 0; i < size; ++i) {
                innerVector->add_dt_bool(dataVec[i] == 0 ? false : true);
            }
            pbVector.set_allocated_bool_vec(innerVector);
            break;
        }
        case DT_CHAR:
        case DT_SHORT:
        ...
        case DT_SECOND:
        case DT_DATETIME: {
            ddbprotobuf::intVector *innerVector = new ddbprotobuf::intVector();
            int size = ddbVector->size();
            vector<int> dataVec(size);
            ddbVector->getInt(0, size, dataVec.data());
            for (int i = 0; i < size; ++i) {
                innerVector->add_dt_int(dataVec[i]);
            }
            pbVector.set_allocated_int_vec(innerVector);
            break;
        }
        case DT_LONG:
        case DT_TIMESTAMP:
        case DT_NANOTIME:
        case DT_NANOTIMESTAMP: {
            ddbprotobuf::longVector *innerVector = new ddbprotobuf::longVector();
            int size = ddbVector->size();
            vector<long long> dataVec(size);
            ddbVector->getLong(0, size, dataVec.data());
            for (int i = 0; i < size; ++i) {
                innerVector->add_dt_long(dataVec[i]);
            }
            pbVector.set_allocated_long_vec(innerVector);
            break;
        }
        case DT_FLOAT: {
            ddbprotobuf::floatVector *innerVector = new ddbprotobuf::floatVector();
            int size = ddbVector->size();
            vector<float> dataVec(size);
            ddbVector->getFloat(0, size, dataVec.data());
            for (int i = 0; i < size; ++i) {
                innerVector->add_dt_float(dataVec[i]);
            }
            pbVector.set_allocated_float_vec(innerVector);
            break;
        }
        case DT_DOUBLE: {
            ddbprotobuf::doubleVector *innerVector = new ddbprotobuf::doubleVector();
            int size = ddbVector->size();
            vector<double> dataVec(size);
            ddbVector->getDouble(0, size, dataVec.data());
            for (int i = 0; i < size; ++i) {
                innerVector->add_dt_double(dataVec[i]);
            }
            pbVector.set_allocated_double_vec(innerVector);
            break;
        }
        case DT_STRING:
        case DT_SYMBOL: {
            ddbprotobuf::stringVector *innerVector = new ddbprotobuf::stringVector();
            int size = ddbVector->size();
            vector<char *> dataVec(size);
            ((ConstantSP)ddbVector)->getString(0, size, dataVec.data());
            for (int i = 0; i < size; ++i) {
                innerVector->add_dt_string(dataVec[i]);
            }
            pbVector.set_allocated_string_vec(innerVector);
            break;
        }
        default:
            throw RuntimeException(ENCODERDECODER_PREFIX + "can not serialize " +
                                   Util::getDataTypeString(ddbVector->getType()) + " type vector.");
    }
...
static ddbprotobuf::Dictionary protobufSerialize(const DictionarySP &ddbDictionary) {
    ...
    pbDict.set_allocated_values(
        new ddbprotobuf::Vector(protobufSerialize(static_cast<VectorSP>(ddbDictionary->values()))));
    return pbDict;
}

ConstantSP dolphinDBprotobufFormatter(const ConstantSP &obj) {
    string data;
    ...
    switch (obj->getForm()) {
        ...
        case DF_DICTIONARY:
            ...
            break;
        default:
            throw RuntimeException(ENCODERDECODER_PREFIX + "can not serialize " + Util::getDataFormString(obj->getForm()) +
                                   " form");
    }
...
//  if pass in table, return one row string vector
//  if pass in anyVector, return multi-row string vector
//  do not support
ConstantSP formatProtobufDDB(int mode, ConstantSP data) {
    // TODO
    // different mode format

    VectorSP ret = Util::createVector(DT_STRING, 0, 1);
    if (data->getForm() == DF_TABLE) {
        ret->append(dolphinDBprotobufFormatter(data));
    } else if (data->getForm() == DF_VECTOR && data->getType() == DT_ANY) {
        for (int i = 0; i < data->size(); ++i) {
            if (data->get(i)->getForm() != DF_TABLE) {
                throw RuntimeException(ENCODERDECODER_PREFIX +
                                       "encode object should be a ddb table or an anyVector only has table elements");
            }
            ret->append(dolphinDBprotobufFormatter(data->get(i)));
        }
    } else {
        throw RuntimeException(ENCODERDECODER_PREFIX + "encode object should be a ddb table.");
    }
    return ret;
}
...
static ConstantSP unserialize(const ddbprotobuf::Scalar &pbScalar) {
    ...
    switch (pbScalar.type_case()) {
        ...
        case ddbprotobuf::Scalar::kDtStringFieldNumber:
            ...
            break;
        default:
            throw RuntimeException(ENCODERDECODER_PREFIX + "unknown field number in Scalar");
    }
    if (pbScalar.is_null()) {
        ddbScalar->setNull();
    }
...
static VectorSP unserialize(const ddbprotobuf::Vector &pbVector, bool isTableColumn = false) {
    ...
    switch (pbVector.type_case()) {
        case ddbprotobuf::Vector::TypeCase::kBoolVec: {
            ddbprotobuf::boolVector innerVec = pbVector.bool_vec();
            bool *data = innerVec.mutable_dt_bool()->mutable_data();
            int size = innerVec.dt_bool_size();
            vector<char> dataBool(size);
            for (int i = 0; i < size; ++i) {
                dataBool[i] = data[i] ? 1 : 0;
            }
            ddbVector = Util::createVector(DT_BOOL, 0, size);
            ddbVector->appendBool((const char *)(dataBool.data()), size);
            break;
        }
        case ddbprotobuf::Vector::TypeCase::kIntVec: {
            ddbprotobuf::intVector innerVec = pbVector.int_vec();
            int *data = innerVec.mutable_dt_int()->mutable_data();
            int size = innerVec.dt_int_size();
            ddbVector = Util::createVector(DT_INT, 0, size);
            ddbVector->appendInt((const int *)data, size);
            break;
        }
        case ddbprotobuf::Vector::TypeCase::kLongVec: {
            ddbprotobuf::longVector innerVec = pbVector.long_vec();
            long *data = innerVec.mutable_dt_long()->mutable_data();
            int size = innerVec.dt_long_size();
            ddbVector = Util::createVector(DT_LONG, 0, size);
            ddbVector->appendLong((const long long *)data, size);
            break;
        }
        case ddbprotobuf::Vector::TypeCase::kFloatVec: {
            ddbprotobuf::floatVector innerVec = pbVector.float_vec();
            float *data = innerVec.mutable_dt_float()->mutable_data();
            int size = innerVec.dt_float_size();
            ddbVector = Util::createVector(DT_FLOAT, 0, size);
            ddbVector->appendFloat((const float *)data, size);
            break;
        }
        case ddbprotobuf::Vector::TypeCase::kDoubleVec: {
            ddbprotobuf::doubleVector innerVec = pbVector.double_vec();
            double *data = innerVec.mutable_dt_double()->mutable_data();
            int size = innerVec.dt_double_size();
            ddbVector = Util::createVector(DT_DOUBLE, 0, size);
            ddbVector->appendDouble((const double *)data, size);
            break;
        }
        case ddbprotobuf::Vector::TypeCase::kStringVec: {
            ddbprotobuf::stringVector innerVec = pbVector.string_vec();
            int size = innerVec.dt_string_size();
            vector<char *> stringData(size);
            for (int i = 0; i < size; ++i) {
                stringData[i] = const_cast<char *>(innerVec.mutable_dt_string(i)->c_str());
            }
            ddbVector = Util::createVector(DT_STRING, 0, size);
            ddbVector->appendString((const char **)(stringData.data()), size);
            break;
        }
        default:
            throw RuntimeException(ENCODERDECODER_PREFIX + "unknown field vector in Vector");
    }
...
ConstantSP dolphinDBprotobufParser(const string &data) {
    ...
    switch (recv.form_case()) {
        ...
        default:
            throw RuntimeException(ENCODERDECODER_PREFIX + "unknown field number in DolphinMessage");
    }
}
ConstantSP parseProtobufDDB(Heap *heap, VectorSP data) {
    if (data->size() == 0) {
        throw RuntimeException(ENCODERDECODER_PREFIX + "no input data");
    } else {
        ConstantSP ret = dolphinDBprotobufParser(data->get(0)->getString());
        if (ret->getForm() != DF_TABLE) {
            throw RuntimeException(ENCODERDECODER_PREFIX + "Not a ddb table protobuf");
        }
        for (int i = 1; i < data->size(); ++i) {
            ConstantSP item = dolphinDBprotobufParser(data->get(i)->getString());
            ...
            INDEX index = ((TableSP)ret)->rows();
            ((TableSP)ret)->append(values, index, errMsg);
            if (errMsg != "") {
                throw RuntimeException(ENCODERDECODER_PREFIX + "different table schema, can't append.");
            }
        }
        return ret;
    }
...
ConstantSP formatProtobufDynamic(string schemaPath, int mode, ConstantSP data) { return new Void(); }
