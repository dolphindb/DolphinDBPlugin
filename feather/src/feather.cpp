...
Status transCol(ConstantSP& retCol, std::shared_ptr<ChunkedArray> chunks, std::shared_ptr<DataType> type) {
    ...
    case DT_CHAR:
    {
        ...
        ((VectorSP)retCol)->appendChar((char*)(colVec.data()), length);
        break;
    }
    case DT_SHORT:
    {
        if(type->id() == arrow::Type::type::UINT8) {
            ...
            ((VectorSP)retCol)->appendShort((short*)(colVec.data()), length);
            break;
        }
        std::vector<short> colVec;
        std::vector<int64_t> nullVec;
    ...
    case DT_INT:
    {
        if(type->id() == arrow::Type::type::UINT16) {
            ...
            ((VectorSP)retCol)->appendInt((int*)(colVec.data()), length);
            break;
        }
        std::vector<int> colVec;
        std::vector<int64_t> nullVec;
    ...
    case DT_LONG:
    {
        if(type->id() == arrow::Type::type::UINT32) {
            ...
            ((VectorSP)retCol)->appendLong((long long*)(colVec.data()), length);
            break;
    ...
    case DT_FLOAT:
    {
        ...
        ((VectorSP)retCol)->appendFloat((float*)(colVec.data()), length);
        break;
    }
    case DT_DOUBLE:
    {
        ...
        ((VectorSP)retCol)->appendDouble((double*)(colVec.data()), length);
    ...
    case DT_STRING:
    {
        ...
        for(int j = 0; j < chunks->num_chunks(); j++) {
            ...
            for(int i = 0; i < arrayLength; i++) {
                ...
                colVec[pos] = (char *)malloc(strLen+1);
                colVec[pos][strLen] = '\0';
                if(strLen != 0) {
                    memcpy(colVec[pos], rawPtr + offsets[i], strLen);
                } else {
                    colVec[pos] = nullStr;
                }
        ...
        ((VectorSP)retCol)->appendString((char**)colVec.data(), length);
        break;
    }
    case DT_DATE:
    {
...
Status loadFromFeather(string filePath, VectorSP columnToRead, TableSP& table) {
    ...
    if (!fp){
        throw RuntimeException("Can't open file " + filePath);
    }
    ...
    fseek(fp, 0, SEEK_SET);
    dataVec.resize(file_len);
    unsigned char* src = dataVec.data();
    int bytes_read = fread(src, 1, file_len, fp);
    if(bytes_read < 0) {
        return Status(StatusCode::IOError, " Error occurred while reading file " + filePath);
    }
    fclose(fp);

    std::shared_ptr<arrow::Buffer> buf(new arrow::Buffer(dataVec.data(), file_len));
...
Status getSchema(string filePath, TableSP& table) {
    ...
    int bytes_read = fread(src, 1, file_len, fp);
    if(bytes_read < 0) {
        throw RuntimeException("Error occurred while reading file " + filePath);
    }
    fclose(fp);
