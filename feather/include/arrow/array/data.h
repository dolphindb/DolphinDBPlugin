...
struct ARROW_EXPORT ArrayData {
  ...
  template <typename T>
  const T* GetValues(int i, int64_t absolute_offset) const {
    if (buffers[i]) {
  ...
  template <typename T>
  const T* GetValuesSafe(int i, int64_t absolute_offset) const {
  ...
  template <typename T>
  const T* GetValuesSafe(int i) const {
    return GetValuesSafe<T>(i, offset);
  }

  // Access a buffer's data as a typed C pointer
  template <typename T>
  T* GetMutableValues(int i, int64_t absolute_offset) {
    if (buffers[i]) {
...
struct ARROW_EXPORT ArraySpan {
  ...
  // Access a buffer's data as a typed C pointer
  template <typename T>
  T* GetValues(int i, int64_t absolute_offset) {
