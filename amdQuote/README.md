...
* 新增支持接收委托表（order）和成交表（trade ）按交易所原始频道代码（ChannelNo）多线程异步写入 server 中的目标表。同一个原始频道代码的委托表和成交表将写入同一张表，且保证其写入顺序。（**2.00.10**）
* 接口 amdQuote::connect 新增支持参数 *outputElapsed*，用于统计插件内部的时延。（**2.00.10**）

## 故障修复

* 修复在订阅一个错误市场后，无法使用 amdQuote::unsubscribe 取消该订阅的问题。（**2.00.10**）
* 修复了在长时间订阅后，行情数据偶现无法正确写入流表的问题。（**2.00.10**）
