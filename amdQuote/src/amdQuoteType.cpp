...
MetaTable AmdNEEQSnapshotTableMeta = {
    {
        "marketType", "securityCode", "origTime", "preClosePrice", "openPrice",
        "lastPrice", "totalVolumeTrade", "totalValueTrade", "numTrades", "highPrice",
        "lowPrice", "priceEarningRatio1", "priceEarningRatio2", "change1", "change2",
        "openInterest", "bidPrice1", "bidPrice2", "bidPrice3", "bidPrice4",
        ...
        "offerVolume5", "indexFactor", "tradingPhaseCode", "varietyCategory",
    },
    {
        DT_INT, DT_STRING, DT_TIMESTAMP, DT_LONG, DT_LONG,
        DT_LONG, DT_LONG, DT_LONG, DT_LONG, DT_LONG,
        DT_LONG, DT_LONG, DT_LONG, DT_LONG, DT_LONG,
        DT_LONG, DT_LONG, DT_LONG, DT_LONG, DT_LONG,
        DT_LONG, DT_LONG, DT_LONG, DT_LONG, DT_LONG,
        DT_LONG, DT_LONG, DT_LONG, DT_LONG, DT_LONG,
        DT_LONG, DT_LONG, DT_LONG, DT_LONG, DT_LONG,
        DT_LONG, DT_LONG, DT_STRING, DT_CHAR,
    }
};
...
//   真实价格*100000   股数   买卖方向   冗余列    冗余列    逐笔数据序号   原始频道代码   数据接收时间戳
...

    {
        ...
        DT_LONG, DT_LONG, DT_INT, DT_LONG, DT_LONG, DT_LONG, DT_INT, DT_NANOTIMESTAMP,
    }
};
...
void initSecurityCodeToIntTypeContainer(MarketTypeContainer<>& container){
    ...
    auto neeqMeta = AmdNEEQSnapshotTableMeta;
    neeqMeta.colTypes_[1] = DT_INT;
    container.add("index", indexMeta);
    container.add("NEEQSnapshot", neeqMeta);
...
void initTypeContainer(MarketTypeContainer<>& container){
    ...
    container.add("NEEQSnapshot", AmdNEEQSnapshotTableMeta);
...
void neeqSnapshotReader(vector<ConstantSP> &buffer, timeMDNEEQSnapshot &data, bool securityCodeToInt) {
    ...
    string securityCode = data.neeqSnapshot.security_code;
    if(securityCodeToInt) {
        int num = std::atoi(securityCode.c_str());
        appendInt(iter++, num);
    } else {
    ...
    appendLong(iter++, data.neeqSnapshot.last_price);
    appendLong(iter++, data.neeqSnapshot.total_volume_trade);
    appendLong(iter++, data.neeqSnapshot.total_value_trade);
    appendLong(iter++, data.neeqSnapshot.num_trades);
    appendLong(iter++, data.neeqSnapshot.high_price);

    appendLong(iter++, data.neeqSnapshot.low_price);
    appendLong(iter++, data.neeqSnapshot.price_earning_ratio1);
    appendLong(iter++, data.neeqSnapshot.price_earning_ratio2);
    appendLong(iter++, data.neeqSnapshot.change1);
    appendLong(iter++, data.neeqSnapshot.change2);

    appendLong(iter++, data.neeqSnapshot.open_interest);
    appendLong(iter++, data.neeqSnapshot.bid_price[0]);
    appendLong(iter++, data.neeqSnapshot.bid_price[1]);
    appendLong(iter++, data.neeqSnapshot.bid_price[2]);
    appendLong(iter++, data.neeqSnapshot.bid_price[3]);

    appendLong(iter++, data.neeqSnapshot.bid_price[4]);
    appendLong(iter++, data.neeqSnapshot.bid_volume[0]);
    appendLong(iter++, data.neeqSnapshot.bid_volume[1]);
    appendLong(iter++, data.neeqSnapshot.bid_volume[2]);
    appendLong(iter++, data.neeqSnapshot.bid_volume[3]);

    appendLong(iter++, data.neeqSnapshot.bid_volume[4]);
    appendLong(iter++, data.neeqSnapshot.offer_price[0]);
    appendLong(iter++, data.neeqSnapshot.offer_price[1]);
    appendLong(iter++, data.neeqSnapshot.offer_price[2]);
    appendLong(iter++, data.neeqSnapshot.offer_price[3]);

    appendLong(iter++, data.neeqSnapshot.offer_price[4]);
    appendLong(iter++, data.neeqSnapshot.offer_volume[0]);
    appendLong(iter++, data.neeqSnapshot.offer_volume[1]);
    appendLong(iter++, data.neeqSnapshot.offer_volume[2]);
    appendLong(iter++, data.neeqSnapshot.offer_volume[3]);

    appendLong(iter++, data.neeqSnapshot.offer_volume[4]);
    appendLong(iter++, data.neeqSnapshot.index_factor);
    appendString(iter++, data.neeqSnapshot.trading_phase_code);
    appendChar(iter++, data.neeqSnapshot.variety_category);
}
...
void orderExecutionReader(vector<ConstantSP> &buffer, MDOrderExecution &data, int seqCheckMode,
    ...
    if(data.orderOrExecution) {
        ...
        appendLong(iter++, data.reachTime);
    } else {
