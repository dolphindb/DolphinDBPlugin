...
unordered_set<string> optionsSet = {"DailyIndex", "ReceivedTime", "StartTime"};
    ...
    if (arguments.size() > 4) {
        ...
        for(int i = 0; i < options->size(); ++i) {
            ...
            if(optionsSet.count(str) == 0)
                throw IllegalArgumentException(__FUNCTION__, "key of options must be 'ReceivedTime', 'DailyIndex', 'StartTime'");
        }
...
ConstantSP subscribe(Heap *heap, vector<ConstantSP> &arguments) { // amdHandler type(snapshot execution order) table marketType codeList
    ...
    if ((transform.isNull() || transform->isNull()) && checkSchema(type, table) == false) {
    ...
    if(dailyIndexFlag){
        ...
        long long now = Util::getEpochTime() + 8 * 3600 * 1000;
        long long diff = now % dateTimestamp;
        dailyIndexStartTimesParam = now / dateTimestamp * dateTimestamp;
        if(diff >= startTime){
            dailyIndexStartTimesParam += dateTimestamp;
        }
    }
    LOG_INFO("[PluginAmdQute]: subscribe " + std::to_string(marketType) + " " + type + " after timestamp " + std::to_string(dailyIndexStartTimesParam));
    if (type == "snapshot") {
        amdQuotePtr->subscribeSnapshot(marketType, table, codeList, transform, receivedTimeFlag, dailyIndexStartTimesParam);
    } else if (type == "execution") {
        amdQuotePtr->subscribeExecution(marketType, table, codeList, transform, receivedTimeFlag, dailyIndexStartTimesParam);
    } else if (type == "order") {
        amdQuotePtr->subscribeOrder(marketType, table, codeList, transform, receivedTimeFlag, dailyIndexStartTimesParam);
    } else if (type == "index") {
        amdQuotePtr->subscribeIndex(marketType, table, codeList, transform, receivedTimeFlag, dailyIndexStartTimesParam);
    } else if (type == "orderQueue") {
        amdQuotePtr->subscribeOrderQueue(marketType, table, codeList, transform, receivedTimeFlag, dailyIndexStartTimesParam);
    } else if (type == "fundSnapshot") {
        amdQuotePtr->subscribeFundSnapshot(marketType, table, codeList, transform, receivedTimeFlag, dailyIndexStartTimesParam);
    } else if (type == "fundExecution") {
        amdQuotePtr->subscribeFundExecution(marketType, table, codeList, transform, receivedTimeFlag, dailyIndexStartTimesParam);
    } else if (type == "fundOrder") {
        amdQuotePtr->subscribeFundOrder(marketType, table, codeList, transform, receivedTimeFlag, dailyIndexStartTimesParam);
    } else if (type == "bondSnapshot") {
        amdQuotePtr->subscribeBondSnapshot(marketType, table, codeList, transform, receivedTimeFlag, dailyIndexStartTimesParam);
    } else if (type == "bondExecution") {
        amdQuotePtr->subscribeBondExecution(marketType, table, codeList, transform, receivedTimeFlag, dailyIndexStartTimesParam);
    } else if (type == "bondOrder") {
        amdQuotePtr->subscribeBondOrder(marketType, table, codeList, transform, receivedTimeFlag, dailyIndexStartTimesParam);
    } else {
...
ConstantSP getSchema(Heap *heap, vector<ConstantSP> &arguments) { // type
    ...
    if (amdDataType == "snapshot") {
        table = getSnapshotSchema(receivedTimeFlag, dailyIndexFlag);
    } else if (amdDataType == "execution") {
        table = getExecutionSchema(receivedTimeFlag, dailyIndexFlag);
    } else if (amdDataType == "order") {
        table = getOrderSchema(receivedTimeFlag, dailyIndexFlag);
    } else if (amdDataType == "index") {
        table = getIndexSchema(receivedTimeFlag, dailyIndexFlag);
    } else if (amdDataType == "orderQueue") {
        table = getOrderQueueSchema(receivedTimeFlag, dailyIndexFlag);
